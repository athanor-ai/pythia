...
| pythia-portfolio-use | gross_ge_abs_net (Finset.abs_sum_le_sum_abs), use_within_limit (div_le_iff) | net_use_identity (:= h) |
...
The 24 real-proof crates are the demo-ready show. The mixed crates are partially verified. The scaffolding crates exercise the Rust implementation (useful as tests) but are not formally verified.
